...
function engSection(data) {
  ...
  return [
    ...
    '- 压测: ' + m.perfMs + ' ms / ' + m.perfFrames + ' steps（预算 ' + m.perfBudgetMs + ' ms）',
    '- 规矩文件: ' + m.rulesLines + ' 行',
...
function webSection(data) {
  ...
  return [
    ...
    '',
    '- 画布: ' + m.canvas,
    '- 浏览器固定帧摘要: `' + m.browserDigest + '`，Node 同帧摘要: `' + m.nodeDigest + '`',
    ...
    '- 菜单遮罩像素: ' + m.menuPixels + '/' + m.expectedMenuPixels + '，死亡横带 ' + m.deadPixels + '/' + m.expectedDeadPixels,
